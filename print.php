<?php
	...
	function get_seance($groupe_id, $emploi_jour, $emploi_temp, $emploi_semestre)
	{
		global $con;
		
		$query = mysqli_query($con, "SELECT * FROM ge_emplois where groupe_id = '".$groupe_id."' and emploi_temp = '".$emploi_temp."' and emploi_jour = '".$emploi_jour."' and emploi_semestre = '".$emploi_semestre."'");
	...
	$query = mysqli_query($con, "SELECT *, (select specialite_nom from ge_specialites where ge_specialites.specialite_id = ge_groupes.specialite_id) as speciliate FROM ge_groupes where specialite_id = '".$_GET["id"]."'");
			...
			foreach( $list_days as $single_value )
			{
				$resultat .= '
					<tr>
						...
						<td align="center"> '.get_seance($group_array['groupe_id'], $single_value, "08:30-09:30", $_GET["semstre"]).'</td>
						<td align="center"> '.get_seance($group_array['groupe_id'], $single_value, "09:30-10:30", $_GET["semstre"]).'</td>
						<td align="center"> '.get_seance($group_array['groupe_id'], $single_value, "10:30-11:30", $_GET["semstre"]).'</td>
						<td align="center"> '.get_seance($group_array['groupe_id'], $single_value, "11:30-12:30", $_GET["semstre"]).'</td>
						<td align="center"> '.get_seance($group_array['groupe_id'], $single_value, "12:30-13:30", $_GET["semstre"]).'</td>
						<td align="center"> '.get_seance($group_array['groupe_id'], $single_value, "13:30-14:30", $_GET["semstre"]).'</td>
						<td align="center"> '.get_seance($group_array['groupe_id'], $single_value, "14:30-15:30", $_GET["semstre"]).'</td>
					</tr>
				';
			}
