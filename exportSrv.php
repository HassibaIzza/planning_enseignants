<?php
	...
	function get_seance($enseignant_id, $emploi_jour, $emploi_temp)
	{
		global $con;
		
		$query = mysqli_query($con, "SELECT * FROM ge_emplois where enseignant_id = '".$enseignant_id."' and emploi_temp = '".$emploi_temp."' and emploi_jour = '".$emploi_jour."'");
	...
	$query = mysqli_query($con, "SELECT * FROM ge_enseignants where enseignant_nom = '".$_GET['enseignant_nom']."'");
	
	if( $query->num_rows > 0 )
	{
		$enseignant_array = mysqli_fetch_array($query);
		
		$departement_array = mysqli_fetch_array(mysqli_query($con, "SELECT * FROM ge_departements where departement_id = '".$enseignant_array['departement_id']."'"));
	}
	...
	$pdf->writeHTML('<table width="100%" cellpadding="0" border="0">
		...
		<tr>
			<td rowspan="2">
				Enseignant: '.$enseignant_array['enseignant_nom'].' '.$enseignant_array['enseignant_prenom'].'<br />
	...
	foreach( $list_days as $single_value )
	{
		$resultat .= '
			<tr>
				...
				<td align="center">'.get_seance($enseignant_array['enseignant_id'], $single_value, "08:30-09:30").'</td>
				<td align="center">'.get_seance($enseignant_array['enseignant_id'], $single_value, "09:30-10:30").'</td>
				<td align="center">'.get_seance($enseignant_array['enseignant_id'], $single_value, "10:30-11:30").'</td>
				<td align="center">'.get_seance($enseignant_array['enseignant_id'], $single_value, "11:30-12:30").'</td>
				<td align="center">'.get_seance($enseignant_array['enseignant_id'], $single_value, "12:30-13:30").'</td>
				<td align="center">'.get_seance($enseignant_array['enseignant_id'], $single_value, "13:30-14:30").'</td>
				<td align="center">'.get_seance($enseignant_array['enseignant_id'], $single_value, "14:30-15:30").'</td>
			</tr>
		';
	}
